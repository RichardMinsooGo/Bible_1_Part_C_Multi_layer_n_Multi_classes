...
class FeedForward():
    ...
    def initialize(self):
        ...
        hidden_size_2 = self.sizes[2]
        output_size   = self.sizes[3]
        ...
        params = {
            ...
            "W3": np.random.randn(output_size, hidden_size_2) * np.sqrt(1./hidden_size_2),
            "b3": np.zeros((output_size, 1)) * np.sqrt(1./hidden_size_2)
        }
        return params
    ...
    def backward(self, y, y_hat):
        ...
        dZ3 = y_hat - y.T
        dW3 = (1./m) * np.matmul(dZ3, self.cache["A2"].T)
...
# Sigmoid + Momentum
model = FeedForward(sizes=[input_size, hidden_size_1, hidden_size_1, output_size], activation='sigmoid')
...
# ReLU + SGD
model = FeedForward(sizes=[input_size, hidden_size_1, hidden_size_1, output_size], activation='relu')
